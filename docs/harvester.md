...
It also writes the **home rows** (`home_rank` table → `/home-rows`): **Top Albums**, **Top Videos** and **Top Artists** ranked by distinct-device play reach over a 30-day LIVE window (no backfill — home stays current; sidesteps the Top 50 backfill-freeze), each card resolved to its **artist channel id** and albums' `OLAK5uy_` play-source ids bridged to browseIds so those plays aren't dropped. This block runs on **every** completed tick (album/video reach shifts on days the Top 50 doesn't) and is fully **isolated in a try/catch that never rethrows** — a failure degrades home rows (the app falls back to its scrape) but can never disturb the Top 50 / Trending / Favorites apply. The fourth home row, **Top Community**, is ranked by how much OUR users play each playlist's songs. The generator computes `community_playlist.reachScore` — the **shrunk-mean device-reach of a playlist's whitelisted members** (`Σ s(reach)/(memberCount + COMM_SIZE_PRIOR)`, a coverage proxy, NOT a raw sum that would size-bias toward mega-dumps; reach = MAX(all-time backfill, recent live) per member) — and `homeRows` orders the gated pool by it, with each playlist's OWN YouTube **view count as the fallback** (`viewCount`, set at discovery from the browse header strapline; `harvester/backfill-community-views.mjs` — cache-only — populates existing rows). The row is **gated** to reflect our users rather than raw virality: eligible playlists must run ≥40 min (whitelisted-member runtime) AND contain a song from the data-driven Top 50 / Trending / Favorites. **Fail-safe:** no reach computed → last-good `reachScore` kept; NULL score → view-count order; empty engaged set → runtime-only; the row never empties. (`reachScore` is a *proxy* — community is search-only, rarely opened — until the app tags `community:<id>` plays, which swaps into the same field with no contract change.) Runs twice daily via `deploy/zemer-autoplaylists.timer` (Shabbat-gated). Knobs: `TOP_N=50`, `TRENDING_N=25`,
