...
album (its most-downloaded, play-tiebroken) since downloads come in album bursts, **acapella-excluded**
(shared master set, same as Trending); `DOWNLOADED_N=30`, `DOWNLOADED_MAX_PER_ALBUM=1`). It fetches `GET /stats` (key from `STATS_KEY`, base from `STATS_URL`), scores
...
It also writes the **home rows** (`home_rank` table → `/home-rows`): **Top Albums**, **Top Videos** and **Top Artists** ranked by distinct-device play reach over a 30-day LIVE window (no backfill — home stays current; sidesteps the Top 50 backfill-freeze), each card resolved to its **artist channel id** and albums' `OLAK5uy_` play-source ids bridged to browseIds so those plays aren't dropped. Each row's pool is sized **larger than the app SHOWS** (Albums `HOME_ALBUMS_N=80`, Artists `HOME_ARTISTS_N=60`, Videos `HOME_VIDEOS_N=80` — the video cap never binds, that row is content-limited to the handful of whitelisted music videos with real reach) so the app's client-side `rotateByArtist` has fresh headroom to turn the row over on refresh (it caps the shown count itself). This block runs on **every** completed tick (album/video reach shifts on days the Top 50 doesn't) and is fully **isolated in a try/catch that never rethrows** — a failure degrades home rows (the app falls back to its scrape) but can never disturb the Top 50 / Trending / Favorites apply. The fourth home row, **Top Community**, needs no community-play telemetry — `/home-rows` ranks it live from the corpus by each playlist's OWN YouTube view count (`community_playlist.viewCount`, set at discovery from the browse header strapline; `harvester/backfill-community-views.mjs` — cache-only — populates existing rows), so the generator writes nothing for it. The view-count ranking is **gated** to reflect our users' taste rather than raw YouTube virality: eligible playlists must run ≥40 min (whitelisted-member runtime) AND contain a song from the data-driven Top 50 / Trending / Favorites (which the generator *does* produce) — with a fail-safe to pure view-count if that engaged set is empty. Runs twice daily via `deploy/zemer-autoplaylists.timer` (Shabbat-gated). Knobs: `TOP_N=50`, `TRENDING_N=25`, `HOME_ALBUMS_N=80`, `HOME_ARTISTS_N=60`, `HOME_VIDEOS_N=80`,
