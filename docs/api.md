...
| `GET /radio?kind=artist\|album\|song\|shuffle&seed=…&allowFemale=0&kidZone=1&blockVideos=1&limit=25` and `GET /radio?continuation=<token>` | **Zemer Radio** — corpus-native, whitelist-pure "what plays next" (replaces the app's `YouTube.next()`; see [radio.md](radio.md)). Returns `{tracks:[ZemerTrack], continuation}`. `seed` is a channelId (`artist`), album browseId **or** playlistId (`album`), or videoId (`song`); `shuffle` needs no seed. `ZemerTrack` = `{videoId, title, artist, artistId, thumbnail, durationSec, explicit, isVideo, releaseDate, album:{id,name}\|null}`. **`continuation`** is an **opaque, self-contained token** (kind+seed+flags+rngSeed+offset) — pass it back to get the next page; the queue is **endless, stateless, and deterministic** (same token → same page, survives the cluster + restarts), `null` at the end. Ranks by the validated co-occurrence blend (session + library co-listening + same-artist) over `data/radio-graph.json`, with a **graceful cold-seed fallback** (a seed with no telemetry → its artist's co-listening profile → same-artist catalog → era/class-leaning popularity — never empty, never off-whitelist), a diversity cap (≤2 of one artist in a row), and per-session variety. Content flags + blocked-ids applied **in-engine** (identical to `/search`). `limit` 1–50 (default 25). Bad `kind`/token → 400. |
