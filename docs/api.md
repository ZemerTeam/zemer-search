...
| `GET /home-rows?allowFemale=0&blockVideos=1&kidZone=0` | **Telemetry-ranked home rows** — replaces the app home tab's YouTube-scraped `featuredAlbums`/`featuredVideos`/`featuredPlaylists`. Returns `{topAlbums, topVideos, topArtists, topCommunity}`; the first three are ranked by **distinct-device play reach** over a 30-day live window (computed twice daily by `harvester/auto-playlists.mjs` into the `home_rank` table; served pure-corpus), `topCommunity` by our users' reach over each playlist's songs, view count as fallback (see below). `topAlbums` = `ZemerAlbum` (+ `explicit`, keeps `playlistId`), `topVideos` = `ZemerTrack`, `topArtists` = `ZemerArtist` `{id,name,thumbnail}`, `topCommunity` = `ZemerPlaylist`; **every card exposes the artist channel id** — as `artistId` on album/video cards (whose own id is the album/videoId), and as `id` on artist cards (the card IS the artist) — so the app's content gate + one-per-artist dedup work (it maps names→null ids otherwise). Content flags + blocked-ids applied inside the read (same contract as `/zemer-playlists`); `blockVideos=1` empties `topVideos`; `kidZone` is always `0` from home. A thin/empty row is the app's cue to fall back to its scrape. `topAlbums`/`topVideos`/`topArtists` rank by device reach not raw plays (raw rewards long albums + superfans). **`topCommunity` is ranked by how much OUR users play each playlist's songs** (`community_playlist.reachScore` — a **shrunk-mean of the whitelisted members' device-reach**, `Σ s(reach)/(memberCount+prior)`, a coverage proxy rather than a raw sum so it doesn't size-bias toward mega-dumps; precomputed by `harvester/auto-playlists.mjs`, since the API has no `/stats` handle), with the playlist's OWN **YouTube view count as the fallback** (`viewCount`, used when `reachScore` is NULL — before the generator's first reach pass the row ranks exactly as the view-count version did). It's **GATED to reflect our users' taste, not raw YouTube virality**: a playlist is eligible only when it is (1) **substantial** — ≥40 min of whitelisted-member runtime (summed over corpus `durationSec`; un-harvested members count 0, so the gate is conservative) — and (2) **relevant** — contains at least one **engaged song** (a member appearing in the data-driven **Top 50 / Trending / Favorites**; Favorites folds in downloads; "Year of ‹Y›" is excluded as a calendar rule). **Fail-safe:** an empty reach signal leaves `reachScore` NULL → view-count order; an empty engaged set drops the relevance clause → runtime-only; the row never empties. (Reach is a proxy — community is search-only, rarely *opened* — until the app tags `community:<id>` plays, which swaps in with no contract change.) Survivors pull a wide pool (80) then apply the same community rules as `/community`: **member-survival** (a card is hidden when no member survives the filter), **track-derived cover** (the first surviving member's art, never the curator's image — gotcha #14), and the **femaleOwned hide** (gotcha #7 rule 2 — a female artist's own playlist is hidden when female is blocked even if it holds a male track), yielding up to 16 `ZemerPlaylist` cards `{id,title,artist,thumbnail,songCount}` (`songCount` = post-filter survivors). A view-untagged playlist (`viewCount` NULL) is excluded. |
