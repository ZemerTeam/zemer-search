...
| R8 | **Down-weight RADIO plays in chart reach** — a radio play is only half-chosen (the user picked the seed; the engine picked the track), yet it counts into `topPlays` reach at full weight, and radio is ~29% of all live plays (2026-07-30). **Station airplay is already fully EXCLUDED from `topPlays`/`topArtists`** (zemer-stats, 2026-07-30 — zero per-song choice + a direct `reach^1.6` scheduler feedback loop); radio deserves a partial version: emit radio-sourced reach as a separate axis from `/stats` and let `auto-playlists.mjs` weight it (~½). Not urgent — radio has counted full since launch and the charts read fine; do it as a deliberate re-rank, with a `DRY=1` before/after diff, not as a side effect. | Medium |
