...
| **Exposure dampener activates (#3)** — *before flipping, let chart-screen impressions accumulate a full week; a `DRY=1` diff taken mid-rollout measures the rollout, not the dampener* | Three steps, in order: (a) the **app** ships impression logging on home rows + search together, (b) coverage reaches 60% of playing devices, (c) **we flip `EXPOSURE_DAMPENER=on`** — deliberately, never automatically. **Verify:** the run log prints `exposure dampener ON — coverage N%`; until then it prints exactly why it is off. | App, then us |
