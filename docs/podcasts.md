...
| `GET /podcast-home-rows` | `{featured:[show…], topPodcasts:[show…], trendingEpisodes:[episode…], newShows:[show…]}` — the Podcasts-tab analogue of `/home-rows`, all rows telemetry-driven with a self-fading cold-start seed (full music parity, nothing stays hardcoded). **`featured`** (hero cut) = TELEMETRY top shows lead; the curated `data/podcast-featured.json` only FILLS while telemetry is thin and fades as plays accrue. **`topPodcasts`** = telemetry-ranked (`data/podcast-surfaces.json`), **ONE-per-channel** (varied, not a wall of masechtos), verified-first + episode-count cold-start. **`trendingEpisodes`** = telemetry-trending, tail filled with newest episodes (recency) while thin, one per show. **`newShows`** = first-seen order (`podcast_show.firstSeenAt`). **CROSS-ROW deduped** (a show appears in at most one row, precedence Featured > Top > Trending > New). `?k=` caps each row; empty row → `[]` (app hides it) |
