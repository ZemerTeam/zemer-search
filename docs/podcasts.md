...
| `GET /podcast-home-rows` | `{featured:[show…], topPodcasts:[show…], trendingEpisodes:[episode…], newShows:[show…]}` — the Podcasts-tab analogue of `/home-rows`. **`featured`** = a curated editorial row (`data/podcast-featured.json`, hand-picked, leads until telemetry matures). **`topPodcasts`** = telemetry-ranked from `data/podcast-surfaces.json`, **ONE-per-channel** (varied, not a wall of masechtos), verified-first + episode-count cold-start. **`trendingEpisodes`** = telemetry-trending, tail filled with newest episodes (recency) while thin. **`newShows`** = first-seen order (`podcast_show.firstSeenAt`). Telemetry always LEADS; the cold-start fallbacks shrink as plays accrue. Channel-gated + content-filtered; `?k=` caps each row; empty row → `[]` (app hides it) |
