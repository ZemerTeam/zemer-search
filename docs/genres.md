...
| `GET /genres?id=<slug>` | The genre as a BROWSABLE PAGE, like an artist page: `{genre:{id,title,kind,trackCount,artistCount,albumCount,singleCount}, artists[], albums[], singles[], songs[], videos[], offset, nextOffset}`. Artists and releases are derived from the surviving member tracks, so every count shown is reachable. An album counts for a genre when **≥2 of its members** carry it — one stray member is not a genre. `k` (≤60) caps the artist/release lists, `limit`/`offset` page the tracks. 404 on an unknown or fully-filtered genre. |
