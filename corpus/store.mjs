...
export function openCorpus(file = DB_PATH) {
  ...
  // track.genres: comma-separated Zemer style slugs (acapella, chasidish, dance, calm, instrumental,
  // chazzanus, wedding, kids, israeli, english, mizrachi, yiddish, carlebach, yemenite). ALBUM-anchored:
  // a genre is a property of a RELEASE, and album identity is what distinguishes an acapella cut from the
  // regular one — track-level guessing cannot (both run the same length). Applied from the durable
  // data/song-genres.json (harvester/song-genres.mjs); the harvest never writes it, so re-harvest keeps it.
