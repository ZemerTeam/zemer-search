...
//
// IMPORTANT (verified against InnerTube.kt ytClient): both calls run with setLogin=false — NO cookie,
// NO SAPISIDHASH Authorization. Search is unauthenticated. We additionally send NO visitorData: a
// stale/flagged shared visitorData causes empty results (see commit df162b8); the WEB_REMIX client
// headers alone work reliably.
...
const HEADERS = {
  ...
  // deliberately no cookie, no Authorization, no X-Goog-Visitor-Id (setLogin=false, no visitorData)
};
...
export async function postSearch({ query = null, params = null, continuation = null }) {
  const body = { context: CONTEXT, query, params };
...
export async function postSuggestions({ input }) {
  const body = { context: CONTEXT, input };
