...
| `server/` | `api.mjs` (HTTP API + cluster + LRU cache; `/search` `/artist` `/album` `/playlist` `/new` `/community` **`/zemer-playlists`** (Zemer-curated categories — the app's "Zemer playlists" section; card list + `?id=` detail, pure corpus reads, content filters + blocked-ids applied inside the read, zero-survivor playlists hidden/404; `thumbnail` = the **generated SVG text cover** `/zemer-playlists/cover?id=…` (branded title card, **fixed-size wrapped title** + a **stable, vivid, well-separated hue keyed to the playlist id** — known lists get hand-picked maximally-spread hues, others a golden-angle hash; stable per id so covers never collide or shift color when the set changes) — never a member's album art) `/health`+`maintenance`), `ui.html` (web UI: search chips + **Community** chip (browse-all, no search) + **New Releases** chip + live refresh-progress bar; renders song **durations**+**play counts**, album **`N songs · MM min`**, numbered album-detail tracklists). |
