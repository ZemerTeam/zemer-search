...
| `corpus/store.mjs` | **SQLite** schema + store API (incl. **`home_rank`** — telemetry-ranked home rows (top albums/videos/artists) served by `/home-rows`; the fourth row, **Top Community**, is ranked by `community_playlist.reachScore` (shrunk-mean device-reach of a playlist's members — how much OUR users play its songs — precomputed by auto-playlists.mjs) with `viewCount` as the fallback, and GATED to our users' taste: eligible playlists must run ≥40 min of whitelisted runtime AND contain an engaged song (a member in the data-driven Top 50 / Trending / Favorites); fail-safe to view-count when reach is absent. A proxy until the app tags community:<id> plays) (artist/track/album/playlist/album_track **+ community_playlist/community_playlist_track + zemer_playlist/zemer_playlist_item** (Zemer-curated categories); `track` carries **`durationSec`/`playCount`**; album `type`/`trackCount`/`totalDurationSec` are read-time aggregates). |
