...
| `index/` | `normalize.mjs` (skeleton + Damerau + `skeletonKey`), **`search.mjs`** (the matcher), `synonyms.mjs`, `categories.mjs` (grouped/by-category search), **`credits.mjs`** (featuring female detection — `buildFemaleMatcher`/`isFemaleInvolved`; whole-token + cross-script-only skeleton, whitelist-validated), **`build-subset.mjs`** (the on-device fallback SNAPSHOT — full read-layer parity minus playback, emitted as content-addressed gzipped **shards + a manifest** into `data/subset/`; the two big tables are hash-bucketed so a single add/remove dirties only a shard or two → the app pulls ONLY changed shards; served by `/subset/manifest` + `/subset/<shard>`), **`station.mjs`** (Zemer STATIONS scheduler — pure-data synchronized-broadcast programming (append semantics; the generator owns the rewrite window): `weight = shrunkReach^1.6+floor × skipDock × (1+3·cooc(prev)) × jitter`, artist-spacing ≥3 slots, half-pool no-repeat, deterministic LCG state; `scheduleAt` = the wall-clock lookup; see [`docs/stations.md`](docs/stations.md)), **`radio.mjs`** (Zemer Radio — corpus-native, whitelist-pure "what plays next", replacing `YouTube.next()`; PURE DATA like the rest of `index/`; ranks by a validated co-occurrence blend `2·session + 1.25·library + 0.2·same-artist + 0.08·artist-tier` over `data/radio-graph.json` (the artist tier = related artists' top tracks from the denser artist×artist graph — bench-pinned BELOW same-artist), ×  a **skip dock** (shrunk listen-through multiplier on every ordering — docked, never excluded), with a same-artist/popularity fallback ladder for cold seeds, diversity cap, and a deterministic stateless continuation; **the seed always leads the queue** (song = THAT song first, artist = one of the artist's own songs, album = the album in order); served by `/radio`; see [`docs/radio.md`](docs/radio.md)), `*.test.mjs`. |
