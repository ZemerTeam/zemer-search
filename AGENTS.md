...
| `server/` | `api.mjs` (HTTP API + cluster + LRU cache; `/search` `/artist` `/album` `/playlist` `/new` `/community` **`/zemer-playlists`** (Zemer-curated categories — the app's "Zemer playlists" section; card list + `?id=` detail, pure corpus reads, content filters + blocked-ids applied inside the read, zero-survivor playlists hidden/404; **`auto-*` detail rows carry additive chart-movement fields** `prevRank`/`delta`/`new` + `playlist.anchorDate` (`server/chart-badges.mjs` — fixed weekly anchor from the rank-history sidecar, raw-rank based so viewer filters can't fabricate movement, absent without history); `thumbnail` = the **generated SVG text cover** `/zemer-playlists/cover?id=…` (branded title card, **fixed-size wrapped title** (drop-shadowed for legibility) + a **stable, vivid color from a validated colorblind-safe distinct palette** (8 colors, worst adjacent CVD ΔE 20 — clearly different, not muddy near-duplicates; keyed to the playlist id so a cover never collides or shifts color when the set changes) — never a member's album art) `/health`+`maintenance`), `ui.html` (web UI: search chips + **Community** chip (browse-all, no search) + **Zemer Playlists** chip (browse the curated+auto lists; auto detail shows ▲/▼/–/NEW chart badges) + **New Releases** chip + live refresh-progress bar; renders song **durations**+**play counts**, album **`N songs · MM min`**, numbered album-detail tracklists). |
